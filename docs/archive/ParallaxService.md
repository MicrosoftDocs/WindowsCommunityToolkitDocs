...
> [!WARNING]
> The ParallaxService is no longer available in the Windows Community Toolkit. Please use the [ParallaxView](/windows/uwp/style/parallax). Read the [Moving to ParallaxView](#parallaxview) section for more info.

The [ParallaxService class](/dotnet/api/microsoft.toolkit.uwp.ui.animations.parallaxservice) allows to create a parallax effect for items contained within an element that scrolls like a ScrollViewer or ListView.
