...
> **Platform APIs:** [StringPool](https://docs.microsoft.com/dotnet/api/microsoft.toolkit.highperformance.buffers.stringpool)

## Syntax

The main entry point for `StringPool` is its `GetOrAdd(ReadOnlySpan<char>)` API, which returns a `string` instance matching the contents of the input [`ReadOnlySpan<char>`](https://docs.microsoft.com/dotnet/api/system.readonlyspan-1), possibly getting the returned object from the internal pool.
